...
?>
  ...
  <div class="p-5 text-center bg-image" style="
    ...
    ">
      ...
      $xsrc="SELECT * FROM studetails WHERE sid=$sid";
        $xrs=mysqli_query($con, $xsrc)or die(mysqli_error($con));
        $xrec=mysqli_fetch_assoc($xrs)
        ?>
        <h2>Suggested Colleges As per Your Rank ['<?php echo $xrec['rank']?>'] in '<?php echo $xrec['eexam']?>'</h2>
      </div>
      
      <?php
      $sid=$_SESSION['sinfo']['sid'];
      $ssrc="SELECT col.*, r.*, stu.* FROM college col INNER JOIN rank r ON r.colid=col.colid INNER JOIN studetails stu ON stu.eexam=r.eexam && r.rankupto>=stu.rank WHERE stu.sid=$sid";
        $srs=mysqli_query($con, $ssrc)or die(mysqli_error($con));
        if(mysqli_num_rows($srs)>0){
        while($srec=mysqli_fetch_assoc($srs)){
        ?> 
            <form action="colfaci.php" method="post" class="form-inline">
              <div class="container-fluid mb-1 mt-2">
                <input type="hidden" name="colid" value="<?php echo $srec['colid']?>">
                <input type="submit" class="form-control btn btn-outline-primary btn-lg" value="<?php echo $srec['cname']?>">
              </div>
            </form>
        <?php
           }
            ...
            echo "No College Match As Per Your Rank";
            }
